...
void main() {
    float noise = pnoise(vWorldPosition * 0.1, vec3(2.0));

    float edge = sin(length(vWorldPosition) * noise + time);
    vec3 c1 = color1 * smoothstep(0.0, 1.0, edge);
    vec3 c2 = color2 * (1.0 - smoothstep(0.0, 1.0, edge));
    vec3 color = c1 + c2;
    ...
    vec4 diffuseColor = vec4(color * diffuse, 1.0);
